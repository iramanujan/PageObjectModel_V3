...
namespace Orange.HRM.Common.Handler.Browser
{
    ...
    public class JavaScript
    {
        ...
        public object WaitTillAjaxLoad(IWebDriver webDriver, int waitTimeInSec = -1)
        {
            try
            {
                Waiter.Wait(webDriver, waitTimeInSec == -1 ? appConfigMember.ObjectTimeout : waitTimeInSec).Until((IWebDriver) =>
                {
                    try
                    {
        ...
        public object WaitTillAjaxLoadIfExists(IWebDriver webDriver, int waitTimeInSec = -1)
        {
            try
            {
                Waiter.Wait(webDriver, waitTimeInSec == -1 ? appConfigMember.ObjectTimeout / 1000 : waitTimeInSec).Until((driver) =>
                {
                    try
                    {
